...
int main(int argc, char **argv) {
	...
	Material red = Material(Vector(0.9,0.1,0), Vector(0,1,0));
	...
	Camera camera(Vector(0,0,1), Vector(1,0,0), Vector(0,0,1), 60*M_PI/180, 900, 1600);
	Scene scene(camera, objects);
	scene.AddLight(Light(Vector(1.7, -1, 2.5), 35));
	scene.Render(8);
	scene.Save("test.bmp");
	return 0;
}
